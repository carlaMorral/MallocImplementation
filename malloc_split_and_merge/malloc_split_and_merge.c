...
p_meta_data request_space(size_t size_bytes) 
{
    ...
    if(size_bytes < 122880) size_bytes = 122880;
...
p_meta_data divide_block(p_meta_data meta_data, size_t size_bytes) 
{
    if (meta_data->size_bytes < size_bytes + 12288){
        return meta_data;
	} 
	p_meta_data sobrant;
	sobrant = (p_meta_data) (((void *) meta_data) + size_bytes + SIZE_META_DATA);

	sobrant->available = 0;
	sobrant->size_bytes = meta_data->size_bytes - size_bytes - SIZE_META_DATA;
	sobrant->previous = meta_data;
	sobrant->next = meta_data->next;
	if (sobrant->next != NULL) sobrant->next->previous = sobrant;
  meta_data->next = sobrant;

	meta_data->size_bytes = SIZE_META_DATA + size_bytes;  
	return meta_data;
}

void fusionar_blocs(p_meta_data meta_data){
  p_meta_data current_right = meta_data;

  while(current_right->next!=NULL && current_right->next->available==1){
    current_right->size_bytes = (current_right->next)->size_bytes;
    current_right->next->size_bytes = 0;
    current_right->next = current_right->next->next;
  }

  if(current_right->next!=NULL){
    current_right->next->previous = current_right;
  }

  p_meta_data current_left = meta_data;

  while(current_left->previous!=NULL && current_left->previous->available==1){
    current_left->size_bytes = (current_left->previous)->size_bytes;
    current_left->previous->size_bytes = 0;
    current_left->previous = current_left->previous->previous;
  }

  if(current_left->previous!=NULL){
    current_left->previous->next = current_left;
  }
...
void *malloc(size_t size_bytes) 
{
  ...
  if (meta_data) { // free block found 
    meta_data->available = 0;
	  ...
	  meta_data = divide_block(meta_data, size_bytes);
